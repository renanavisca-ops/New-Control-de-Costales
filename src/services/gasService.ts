...
class GASService {
  ...
  private getAuthRecords(): AuthRecord[] {
    return this.getMockData<AuthRecord>(MOCK_DB_AUTH);
  }

  private saveAuthRecords(records: AuthRecord[]) {
  ...
  private upsertAuthRecord(email: string, password: string, mustChangePassword: boolean) {
    ...
    if (index >= 0) {
      ...
      records.push({ email: cleanEmail, password, mustChangePassword });
    }
  ...
  private ensureRootAdmin(users: User[]) {
    ...
    if (existing) {
      normalizedUsers = normalizedUsers.map((u) =>
        u.email.toLowerCase() === ROOT_ADMIN_EMAIL
          ? { ...u, rol: Role.ADMIN, email: ROOT_ADMIN_EMAIL }
          : u
      );
    } else {
      ...
      normalizedUsers = [rootAdmin, ...normalizedUsers];
    }

    this.saveMockData(MOCK_DB_USERS, normalizedUsers);
      ...
      setTimeout(() => {
        ...
        switch (action) {
          case 'authUsuario': {
            ...
            const authRecord = this.getAuthRecord(email);
          ...
          case 'createManagedUser': {
            ...
            if (!actor) {
              resolve({ ok: false, error: 'Usuario creador no válido.' });
              break;
            }
          ...
          case 'changePassword': {
            ...
            this.upsertAuthRecord(email, newPassword, false);
  ...
  async auth(email: string, password: string) {
    return this.request('authUsuario', { email, password });
  }
